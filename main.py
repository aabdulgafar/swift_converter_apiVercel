...
@app.post("/convert/mt101/to/pain001")
async def mt101_to_pain001_api(request: Request):
    raw_text = await request.body()
    message = raw_text.decode("utf-8")
    return {"pain001": convert_mt101_to_pain001(clean_input(message))}

@app.post("/convert/mt101/to/json")
async def mt101_to_json_api(request: Request):
    raw_text = await request.body()
    message = raw_text.decode("utf-8")
    return convert_mt101_to_json(clean_input(message))
...
@app.post("/convert/mt103/to/pacs008")
async def mt103_to_pacs008_api(request: Request):
    raw_text = await request.body()
    message = raw_text.decode("utf-8")
    return {"pacs008": convert_mt103_to_pacs008(clean_input(message))}

@app.post("/convert/mt103/to/json")
async def mt103_to_json_api(request: Request):
    raw_text = await request.body()
    message = raw_text.decode("utf-8")
    return convert_mt103_to_json(clean_input(message))
